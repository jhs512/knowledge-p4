# 마스터 노드 인덱스

모든 노드는 생성·이동·삭제 시 이 인덱스를 갱신해야 합니다. 행 형식: `| id | 제목 | namespace | visibility | updated |`

## pillar

| id | 제목 | namespace | visibility | updated |
|---|---|---|---|---|
| [pillar-work-foundation](../pillars/pillar-work-foundation.md) | 업무 지식 기반 | work | public | 2026-06-10 |

## decision

| id | 제목 | namespace | visibility | updated |
|---|---|---|---|---|
| [decision-work-first](../decisions/decision-work-first.md) | 지식 관리를 Infinite Brain vault로 한다 | work | public | 2026-06-10 |

## concept

| id | 제목 | namespace | visibility | updated |
|---|---|---|---|---|

## question

| id | 제목 | namespace | visibility | updated |
|---|---|---|---|---|

## playbook

| id | 제목 | namespace | visibility | updated |
|---|---|---|---|---|
| [playbook-work-springboot-init](../playbooks/playbook-work-springboot-init.md) | 스프링부트 프로젝트 생성 취향 | work | public | 2026-06-10 |

## task

| id | 제목 | namespace | visibility | updated |
|---|---|---|---|---|

## event

| id | 제목 | namespace | visibility | updated |
|---|---|---|---|---|

## pattern

| id | 제목 | namespace | visibility | updated |
|---|---|---|---|---|
| [pattern-work-context-mode-build-redirect](../patterns/pattern-work-context-mode-build-redirect.md) | context-mode 훅의 빌드 명령 리다이렉트 | work | public | 2026-06-10 |
| [pattern-work-windows-git-bash](../patterns/pattern-work-windows-git-bash.md) | Windows + Git Bash 셸 환경 주의사항 | work | public | 2026-06-10 |

## hypothesis

| id | 제목 | namespace | visibility | updated |
|---|---|---|---|---|

## fact

| id | 제목 | namespace | visibility | updated |
|---|---|---|---|---|
| [fact-work-career-it-instructor](../facts/fact-work-career-it-instructor.md) | IT강사 직업에 대한 열정 | work | public | 2026-06-10 |
| [fact-work-motivation-recognition](../facts/fact-work-motivation-recognition.md) | 기술 전파를 통한 인정욕구 충족 | work | public | 2026-06-10 |
| [fact-work-value-earning-recognition](../facts/fact-work-value-earning-recognition.md) | 돈에 대한 가치관: 소유보다 많이 버는 것 | work | public | 2026-06-10 |
| [fact-work-practice-meditation](../facts/fact-work-practice-meditation.md) | 명상 실천 | work | public | 2026-06-10 |
| [fact-work-hobby-futsal](../facts/fact-work-hobby-futsal.md) | 풋살을 즐기는 취미 | work | public | 2026-06-10 |
| [fact-work-food-preference](../facts/fact-work-food-preference.md) | 좋아하는 음식: 순대국밥·콩나물비빔밥·청국장 | work | public | 2026-06-10 |

## source

| id | 제목 | namespace | visibility | updated |
|---|---|---|---|---|

## bookmark

| id | 제목 | namespace | visibility | updated |
|---|---|---|---|---|

## note

| id | 제목 | namespace | visibility | updated |
|---|---|---|---|---|

## contact

| id | 제목 | namespace | visibility | updated |
|---|---|---|---|---|

## reference

| id | 제목 | namespace | visibility | updated |
|---|---|---|---|---|

## custom

| id | 제목 | namespace | visibility | updated |
|---|---|---|---|---|
